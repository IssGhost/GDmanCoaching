...
export default function DashboardAccount() {
  ...
  const recentActivity = [...orders.slice(0, 3).map((item) => ({ id: item._id, title: `Order #${item.number || item._id.slice(-6).toUpperCase()}`, detail: item.status, to: "/dashboard/orders" })), ...requests.slice(0, 3).map((item) => ({ id: item._id, title: item.subject, detail: item.quote?.status === "sent" ? "Quote ready to review" : item.status, to: "/messages" }))].slice(0, 5);

  return <div className="space-y-7">
    <section className="flex flex-col justify-between gap-4 rounded-[2rem] bg-[#12372a] p-6 text-white md:flex-row md:items-center"><div><p className="text-xs font-black uppercase tracking-[.18em] text-[#c6ff4a]">Account overview</p><h2 className="mt-2 text-3xl font-black text-white">Welcome back, {user?.fullName?.split(" ")[0] || "Player"}</h2><p className="mt-2 text-sm leading-6 text-white/75">Your next coaching action, active requests, and account details are organized below.</p></div><Link to="/coaches" className="rounded-xl bg-[#c6ff4a] px-5 py-3 text-center text-sm font-black text-[#12372a]">Find your next coach</Link></section>
    ...
    <section className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]"><div className="rounded-[2rem] border border-[#12372a]/10 bg-white p-6 shadow-sm"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#d9f7fb] text-[#087f73]"><FaUser/></div><div><h3 className="text-xl font-black text-[#12372a]">Account details</h3><p className="text-sm text-[#5f746c]">Keep your contact information current.</p></div></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Full name"><input className="pp-input mt-1 px-4 py-3" value={form.fullName} onChange={(e) => setForm((current) => ({ ...current, fullName: e.target.value }))}/></Field><Field label="Phone"><input className="pp-input mt-1 px-4 py-3" value={form.phone} onChange={(e) => setForm((current) => ({ ...current, phone: e.target.value }))}/></Field></div><button onClick={save} disabled={saving} className="pp-btn-primary mt-5 px-5 py-3">{saving ? "Saving..." : "Save account details"}</button></div>
      <div className="rounded-[2rem] border border-[#12372a]/10 bg-white p-6 shadow-sm"><div className="flex items-center justify-between gap-3"><div><h3 className="text-xl font-black text-[#12372a]">Recent activity</h3><p className="text-sm text-[#5f746c]">Your latest orders and coach conversations.</p></div><Link to="/messages" className="text-sm font-black text-[#087f73]">View requests</Link></div><div className="mt-5 space-y-3">{recentActivity.map((item) => <Link key={`${item.to}-${item.id}`} to={item.to} className="flex items-center justify-between gap-3 rounded-2xl border border-[#12372a]/10 bg-[#fffdf6] p-4 transition hover:bg-[#eaf9f7]"><div><div className="font-black text-[#12372a]">{item.title}</div><div className="mt-1 text-xs font-bold capitalize text-[#5f746c]">{String(item.detail || "pending").replaceAll("_", " ")}</div></div><FaArrowRight className="text-[#087f73]"/></Link>)}{!recentActivity.length && <div className="rounded-2xl bg-[#eaf9f7] p-5 text-sm font-semibold leading-6 text-[#40584f]">No activity yet. Browse coaches to start a standard package or personalized request.</div>}</div></div></section>
